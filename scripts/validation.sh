...
if [ ! -f "./scripts/start.sh" ] || [ ! -f "./scripts/stop.sh" ]; then
...
docker ps | grep -E 'chromadb|postgres|streamlit' | tee -a "$LOGFILE"

# Count our containers
container_count=$(docker ps | grep -E 'chromadb|postgres|streamlit' | wc -l)
...
if [ "$container_count" -lt 3 ]; then
    ...
    if [ "$start_answer" == "y" ] || [ "$start_answer" == "Y" ]; then
        log "Starting system using ./scripts/start.sh"
        ./scripts/start.sh
...
log "To start the system: ./scripts/start.sh"
log "To stop the system: ./scripts/stop.sh"
